...
This exporter sends OpenTelemetry logs and traces to [DuckLake](https://ducklake.select/), a lakehouse storage format that combines Parquet files with PostgreSQL metadata. Data is written to Parquet files and registered in a PostgreSQL metadata catalog following the DuckLake specification, enabling efficient querying with DuckDB.
